...
void ExecutionPanel::updateState(const QString &tick, const QString &upsilon, const QString &beta,
                                 const QString &koppa, bool psiFired, int stackDepth,
                                 const QString &rhoSource, const QString &psiMode) {
    ...
    m_psiLabel->setText(psiFired ? tr("ψ fired (%1)").arg(psiMode) : tr("ψ idle (%1)").arg(psiMode));
...
void ExecutionPanel::appendLogRow(const QStringList &row) {
    ...
    for (int column = 0; column < row.size(); ++column) {
        m_logTable->setItem(rowIndex, column, new QTableWidgetItem(row.at(column)));
    }
...
void ExecutionPanel::buildUi() {
    ...
    m_logTable->setHorizontalHeaderLabels({tr("Tick"), tr("MT"), tr("υ"), tr("β"), tr("κ"), tr("ψ"), tr("ρ"),
                                           tr("Stack")});
